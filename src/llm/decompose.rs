...
拆解建议：
- 树的深度建议 1-3 层，通常无需超过 4 层
- 每个节点子问题建议 2-5 个
- 无需进一步拆解时，children 设为空数组 []
...
[
  {
    "question": "子问题",
    "children": [
      {"question": "孙问题", "children": []}
    ]
  }
]
```"#;

const DEFAULT_STRATEGY: &str = "按以下五维模型选择几个最相关的维度对父问题拆解：\n\
     - core（核心）：问题的核心是什么\n\
     - why（原因）：为什么会有这个问题\n\
     - how（方法）：如何解决这个问题\n\
     - case（案例）：有哪些实际案例\n\
...
impl SubQueryTree {
    pub fn queries(&self) -> Vec<String> {
        ...
        self.collect_queries(0, &mut result);
        result
    }

    fn collect_queries(&self, depth: usize, result: &mut Vec<String>) {
        let q = self.question.trim();
        if q.len() >= 5 {
            result.push(q.to_string());
        } else {
            tracing::debug!("Skipping query with too short question: {}", q.len());
        }
        if depth < MAX_SAFE_DEPTH {
            for child in &self.children {
                child.collect_queries(depth + 1, result);
            }
        }
    }
}
