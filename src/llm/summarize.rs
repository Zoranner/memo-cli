...
/// 流式版本：返回 ChatStream，供调用方逐 token 打印
pub async fn summarize_results_stream(
    client: &LlmClient,
    original_query: &str,
    results: &[QueryResult],
    strategy: Option<&str>,
    ...
    if results.is_empty() {
        return Err(anyhow::anyhow!("No results to summarize"));
    }
    ...
    client.chat_stream(&prompt).await
}
