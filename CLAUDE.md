...
- Show subcommand help: `cargo run -- recall --help`
- Awaken local data/config in default data dir: `cargo run -- awaken`
- Recall from the default local memory space: `cargo run -- recall "your query" --deep`
- Restore derived layers when needed: `cargo run -- restore`
- Run dream immediately: `cargo run -- dream`
...
- `crates/engine`: stateful local memory engine. It owns SQLite persistence, Tantivy text search, HNSW vector search, retrieval ranking, and layer promotion/cooling.
...
`MemoryEngine::recall` in `crates/engine/src/engine.rs` is multi-stage and should be understood before changing ranking behavior.
...
Non-deep recall may auto-escalate to deep search when results look ambiguous. If ranking changes seem surprising, inspect whether auto-escalation or rerank was triggered before changing weights.

## Dream and restore

Dream logic lives in `MemoryEngine::dream` / `run_dream`.
...
- keep dream behavior deterministic when invoked explicitly from CLI or external schedulers
...
Default CLI data dir is `.memo` (`src/main.rs`). `memo awaken` writes local `config.toml` and `providers.toml` templates into that data dir.
...
- `crates/engine/tests/engine_flow.rs` for end-to-end engine behavior, ranking, dream behavior, and index maintenance
