...
- 派生层维护能力位于 `Index Layer` 上方，负责把 `index_jobs` 翻译成 text/vector 更新；当前代码中可继续保留内部 helper，但它不是公开产品命令；
...
- 当前代码可保留内部 `restore` / `restore_full` helper 作为实现复用点，但不再形成公开 `memo restore` 命令。
